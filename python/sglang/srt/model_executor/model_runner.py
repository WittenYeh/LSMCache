...
class ModelRunner:
    ...
    def load_model(self):
        ...
        logger.info(
            f"[ModelRunner::load_model] Load weight end. "
    ...
    def init_memory_pool(
        ...
        # TODO[Witten Yeh]: when enable_kvstore is set true, 
        # we should use DB size to profile max_total_number_tokens
        self.max_total_num_tokens = self.profile_max_num_token(total_gpu_memory)
        
        # TODO[Witten Yeh]: when enable_kvstore is set ture,
        # we should use DB size to profile max_num_reqs
        ...
        if not self.spec_algorithm.is_none():
            ...
            if self.enable_kvstore:
                raise NotImplementedError("[ModelRunner::init_memory_pool] KV Storage Arch does not support spec_algorithm currently")
        ...
        assert not self.enable_kvstore or self.server_args.page_size == 1, \
            "[ModelRunner::init_memory_pool] KV Storage Arch does not support spec_algorithm currently"
        ...
        if self.use_mla_backend:
            ...
            assert not self.enable_kvstore,\
                "[ModelRunner::init_memory_pool] KV Storage Arch does not support double_sparsity backend"
